...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        for (ClientInfo client : clients) {
            //for each client send info to broker
            HttpEntity<ClientInfo> request = new HttpEntity<>(client);
            ClientApplication clientApplication=restTemplate.postForObject("http://localhost:8085/application", request, ClientApplication.class);
            displayProfile(client);
            for(int i=0;i<clientApplication.getQuotations().size();i++)
            displayQuotation(clientApplication.getQuotations().get(i));
        }
    }
